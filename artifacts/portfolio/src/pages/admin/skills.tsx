...
import { Plus, Trash2, X, Save, GripVertical, Zap } from "lucide-react";
...
export default function AdminSkills() {
  ...
  const [form, setForm] = useState({ name: "", category: "Languages", showInHero: false });
  ...
  const handleAdd = (e: React.FormEvent) => {
    e.preventDefault();
    save([...skills, { id: uid(), name: form.name, category: form.category, order: skills.length + 1, showInHero: form.showInHero }]);
    setForm({ name: "", category: "Languages", showInHero: false });
    setModal(false);
  };

  const remove = (id: string) => { save(skills.filter((s) => s.id !== id)); };
  const toggleHero = (id: string) => { save(skills.map((s) => s.id === id ? { ...s, showInHero: !s.showInHero } : s)); };
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">{skills.length} skills total · {skills.filter((s) => s.showInHero).length} shown in hero</p>
          </div>
          <button onClick={() => setModal(true)} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary/90 transition-colors">
              ...
              <div className="space-y-2">
                {items.map((s, i) => (
                  <motion.div key={s.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}
                    ...
                    {s.showInHero && (
                      <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-xs bg-primary/10 text-primary border border-primary/20">
                        <Zap size={9} fill="currentColor" /> Hero
                      </span>
                    )}
                    ...
                    <button onClick={() => toggleHero(s.id)} className={`w-7 h-7 flex items-center justify-center rounded-lg transition-all ${s.showInHero ? "text-primary bg-primary/10" : "text-muted-foreground opacity-0 group-hover:opacity-100 hover:text-primary hover:bg-primary/10"}`} title="Toggle hero visibility">
                      <Zap size={13} fill={s.showInHero ? "currentColor" : "none"} />
                    </button>
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="fixed inset-x-4 top-1/2 -translate-y-1/2 max-w-sm mx-auto bg-card border border-border rounded-2xl shadow-2xl z-[101]">
                ...
                <form onSubmit={handleAdd} className="p-5 space-y-4">
                  <div>
                    ...
                    <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required placeholder="e.g. Next.js"
                  ...
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    <input type="checkbox" checked={form.showInHero} onChange={(e) => setForm({ ...form, showInHero: e.target.checked })} className="w-4 h-4 accent-primary" />
                    <span className="text-sm text-foreground">Show in Hero section</span>
                  </label>
